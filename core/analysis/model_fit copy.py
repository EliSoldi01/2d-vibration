...
def calculate_saturation_point(results, threshold=0.95):
    """
    Calcola l'angolo (in gradi) in cui inizia la saturazione per il modello Tanh.
    
    Args:
        results (dict): Il dizionario restituito da fit_models()
        threshold (float): Soglia dell'asintoto per definire la saturazione (es. 0.90 per il 90%)
        
    Returns:
        float or None: L'angolo abs(x) corrispondente all'inizio della saturazione.
    """
    tanh_res = results.get("tanh", {})
    if not tanh_res.get("ok", False):
        logger.warning("[saturation] Modello Tanh non valido o fit fallito.")
        return None

    L_fit, k_fit = tanh_res["params"]
    
    # Inversione analitica di: L * tanh(k * x) = threshold * L
    # => tanh(k * x) = threshold => x = arctanh(threshold) / k
    x_sat = np.arctanh(threshold) / k_fit
    y_sat = tanh_sigmoid(x_sat, L_fit, k_fit)
    
    print(f"\n== Analisi Saturazione Tanh ({threshold*100:.0f}% dell'asintoto) ==")
    print(f"  Parametri stimati : L = {L_fit:.3f}°, k = {k_fit:.4f}")
    print(f"  Inizio saturazione: x = ±{x_sat:.2f}° (con y = ±{y_sat:.2f}°)")
    print("=================================================================\n")
...
def _fit_one(fn, x, y, weights, p0, bounds, k_params):
    """
    Fit di un modello non lineare con curve_fit.

    Restituisce:
    - params: stime dei parametri
    - params_std: errori standard
    - params_ci95: intervalli di confidenza 95%
    """
    try:
        popt, pcov = curve_fit(
            fn, x, y,
        ...
        z = 1.96
        ci_lower = popt - z * params_std
        ci_upper = popt + z * params_std
        ...
        return {
            ...
            "params_ci95": np.column_stack((ci_lower, ci_upper)),
            "r2_zero": r2["R2_zero"],
            "r2_mean": r2["R2_mean"],
        ...
        logger.warning(
            f"[sigmoid_fit] fit failed for {fn.__name__}: {exc}"
        )

        return {
            "params": np.full(k_params, np.nan),
            "params_std": np.full(k_params, np.nan),
            "params_ci95": np.full((k_params, 2), np.nan),
            "r2_zero": np.nan,
            "r2_mean": np.nan,
            "aic": np.nan,
...
def _print_parameter_ci(results):
    ...
    for model_name, result in results.items():
        ...
        if "params_ci95" not in result:
            continue

        params = result["params"]
        ci = result["params_ci95"]

        print(f"\n{model_name}:")

        for i, (p, (low, high)) in enumerate(zip(params, ci)):
...
def fit_models(x, y, weights=None, plot=True, output_folder=None, labels=None):
    ...
    _print_parameter_ci(results)

    x_sat = calculate_saturation_point(results, threshold=0.95)
...
def plot_grouped_scatter(
    ...
    for _, row in df.iterrows():
        ...
        # 95% CI del valore medio
        if plot_ci and 'real_std' in df.columns and 'N' in df.columns:

            se = row['real_std'] / np.sqrt(row['N'])
            ci = 1.96 * se
    ...
    # Disegna linee di saturazione se il fit Tanh è presente
    if saturation_plot and results.get("tanh", {}).get("ok"):

        x_sat, y_sat = calculate_saturation_point(results, threshold=0.95)

        if x_sat is not None and y_sat is not None:

            # --- Vertical saturation thresholds ---
            ...
            # --- Horizontal saturation thresholds ---
            ax.axhline(
                y_sat,
                color='red',
                linestyle=':',
            ...
            # --- Add saturation threshold to legend ---
            sorted_handles = list(sorted_handles)
            sorted_labels = list(sorted_labels)

            sorted_handles.append(sat_line_pos)
            sorted_labels.append(
                f'Saturation 95% (±{x_sat:.1f}°; ±{y_sat:.1f}°)'
            )
    ...
    # --- stessa scala X/Y e includi le barre di errore ---
    x_min = df["ideal_angle"].min()
    x_max = df["ideal_angle"].max()

    y_min = (df["real_mean"] - df["real_std"]).min()
    y_max = (df["real_mean"] + df["real_std"]).max()
    ...
    ax.set_xlim(-x_max_abs - x_max_abs*0.1, x_max_abs + x_max_abs*0.1)
