...
SHEET_NAME = "Complete"
OUTPUT_DIR = "saturation_test\\saturation_duration_results"
...
EMPIRICAL_PLATEAU_THRESHOLD = 0.30

# Number of consecutive small increments required
EMPIRICAL_PLATEAU_CONSECUTIVE = 1
...
def prepare_effective_duration(df):
    ...
    For triceps, angle magnitude is inverted:

        magnitude = -angle_deg

    because triceps responses are negative in the
    ...
    df["muscle"] = np.nan
    df["n_vibrators"] = np.nan
    df["effective_stimulation_duration"] = np.nan

    # --------------------------------------------------------
    # Biceps
    # --------------------------------------------------------

    for pattern, n_vib in BICEPS_PATTERNS.items():
    ...
    for pattern, n_vib in TRICEPS_PATTERNS.items():
    ...
    pure_df["response_magnitude"] = np.where(
        pure_df["muscle"] == "biceps",
        pure_df[ANGLE_COL],
        -pure_df[ANGLE_COL]
    )
    ...
    for muscle, patterns in [
        ("biceps", BICEPS_PATTERNS),
        ("triceps", TRICEPS_PATTERNS),
    ]:
        ...
        for pattern, n_vib in patterns.items():
...
# ============================================================
# AGGREGATE BY EFFECTIVE DURATION
# ============================================================
...
def aggregate_by_effective_duration(
    ...
    """
    Combine all pure patterns of the same muscle
    according to effective stimulation duration.
    ...
    both contribute to the 3 s effective-duration point.
    """
...
def fit_tanh(
    ...
    the response used here is magnitude.
    """
...
def calculate_empirical_plateau(
    ...
    This prevents an early local flattening from being classified
    as a plateau when the response subsequently increases again.
    """
    ...
    for start_index in range(
        ...
        # ----------------------------------------------------
        # Check whether the required number of consecutive
        # increments is below threshold.
        # ----------------------------------------------------
        ...
        # ----------------------------------------------------
        # Check the rest of the curve.
        #
        # If a later increment exceeds the threshold,
        # the candidate is not considered a sustained plateau.
        # ----------------------------------------------------
...
def plot_individual_patterns(
    ...
    ax.set_title(
        f"{muscle.capitalize()}: individual pure patterns"
    )
    ...
    path = os.path.join(
        OUTPUT_DIR,
        f"{muscle}_individual_patterns.png"
    )
...
def plot_combined_curve(
    ...
    # --------------------------------------------------------
    # Plot all observed data including baseline
    # --------------------------------------------------------
    ...
    # --------------------------------------------------------
    # Fit uses only non-zero durations
    # --------------------------------------------------------
    ...
    ax.set_title(
        f"{muscle.capitalize()}: saturation analysis"
    )
    ...
    path = os.path.join(
        OUTPUT_DIR,
        f"{muscle}_saturation_curve.png"
    )
...
def analyze_muscle(
    ...
    print("=" * 70)
    print(
        f"{muscle.upper()} SATURATION ANALYSIS"
    )
    print("=" * 70)
...
def save_results(
    ...
    with pd.ExcelWriter(
        ...
        if biceps_results is not None:

            biceps_results[
                "grouped"
            ].to_excel(
                writer,
                sheet_name="Biceps_duration",
                index=False
            )
            ...
            biceps_results[
                ...
                writer,
                sheet_name="Biceps_saturation",
                index=False
            )

        if triceps_results is not None:

            triceps_results[
                "grouped"
            ].to_excel(
                writer,
                sheet_name="Triceps_duration",
                index=False
            )
            ...
            triceps_results[
                ...
                writer,
                sheet_name="Triceps_saturation",
                index=False
            )

        if control_results is not None:

            control_results.to_excel(
                writer,
                sheet_name="Control_001_001",
                index=False
            )
...
# ============================================================
# SAVE EFFECTIVE DATASET
# ============================================================

def save_effective_dataset(
    df
):

    output_file = os.path.join(
        OUTPUT_DIR,
        "effective_stimulation_duration_data.xlsx"
    )
    ...
    print(
        f"\nEffective-duration dataset saved to:\n"
        f"{output_file}"
    )
...
def main():
    ...
    pure_df = prepare_effective_duration(
        df
    )

    save_effective_dataset(
        pure_df
    )
    ...
    triceps_results = analyze_muscle(
        pure_df,
        "triceps"
    )
    ...
    print("=" * 70)
    print(
        f"CONTROL: {CONTROL_PATTERN}"
    )
    print("=" * 70)

    control_results = analyze_control(
        df
    )
    ...
    for muscle, results in [
        ("Biceps", biceps_results),
        ("Triceps", triceps_results),
    ]:
